...
txtTitle = 'v8.0_intensive_snn_tests38_conv_BNN_1st_model' + str(0 + 1) + '.txt'
...
txtTitle = 'v8.0_intensive_snn_tests38_weights1' + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
numRow = int(weights[0].size / weights[0][0].size)
numCol = int(weights[0][0].size / weights[0][0][0].size)
numInputSlices = int(weights[0][0][0].size / weights[0][0][0][0].size)
numOutputSlices = int(weights[0][0][0][0].size / weights[0][0][0][0][0].size)
listCnt = np.zeros((numOutputSlices))

# Save each set of weights in the txt format
txtTitle = 'v8.0_intensive_snn_tests38_weights_sorted' + '.txt'
with open(txtTitle, 'w') as outfile:
    weightsSorted = weights[0].reshape(numRow * numCol * numInputSlices * numOutputSlices)
    weightsSorted = weightsSorted.reshape((numOutputSlices, numRow * numCol), order='F')
    for i in range(numOutputSlices):
        outfile.write('\n {0} outputSlices weights \n'.format(i))
        outfile.write('{0}'.format(weightsSorted[i]))
    del(weightsSorted)

# Step 3.1: Find the number of non-zero solutions in each set of weights
for outputSlices in range(numOutputSlices):
    for row in range(numRow):
        for col in range(numCol):
            if(abs(weights[0][row][col][0][outputSlices]) > 0.02):
                listCnt[outputSlices] += 1

# Step 3.2: Calculate the mean and variance of the numbers
mean = np.mean(listCnt)
variance = np.var(listCnt)
stdDeviation = variance * len(listCnt)

# Step 3.3: Find outliers which are far from the zero after rescaling using z-score
listIndexOutliers = []
listIndexNonOutliers = []
for outputSlices in range (len(listCnt)):
    listCnt[outputSlices] = (listCnt[outputSlices] - mean) / (stdDeviation ** 0.5)
    if(listCnt[outputSlices] > 0.5):
        listIndexOutliers.append(outputSlices)
    else:
        listIndexNonOutliers.append(outputSlices)

# Step 4: Replace outliers by non-outliers which are randomly selected
# Define the buffer of weights
tempListWeights = [[[] for i in range(numCol)] for i in range(numRow)]

for l in range(len(listIndexOutliers)):
    # Copy the sparse weights into the buffer
    # Dimension: (row, col, inputSlice, outputSlice)
    k=random.choice(listIndexNonOutliers)
    print("index of non-outliers:" + str(k))
    ...
    k=listIndexOutliers[l]
    for i in range(numRow):
        for j in range(numCol):
            weights[0][i][j][0][k] = listWeightsShuffled[i*numCol+j]
...
# Predict the intermediate results from the Binary Neural Network
BNN_prediction = layer2model.predict(np.asarray([x_test[0]]))
txtTitle = 'v8.0_intensive_snn_tests38_conv_BNN_2nd_model' + str(0 + 1) + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
...
txtTitle = 'v8.0_intensive_snn_tests38_weights2' + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('# Array shape: {0}\n'.format(weights.shape))
...
for r in range(100):
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests38_conv', test_index,
    ...
    sentences += '\n' + 'maxpool layer done'
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests38_maxpool', test_index,
...
################### For debugging purpose, save the weigts in txt format #######################################
txtTitle = 'v8.0_Intensive_snn_tests38.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    #outfile.write('# Array shape: {0}\n'.format(weights.shape))

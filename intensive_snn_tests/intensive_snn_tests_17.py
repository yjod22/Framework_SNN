...
#              Conv(1, 4x4), activation function(tanh(0.7x)), Conv(5, 3x3), activation function(relu(0.1x)),
#              maxPooling(2x2), Dense(100), activation function(relu), Dense(10), activation function(softmax)
#              Stochastic Conv(APC+BTanh), Stochastic Conv(Mux+Relu), Stochastic Dense(APC mode+Relu), BinaryConnectedLAyer
...
model.add(Conv2D(5, kernel_size=(3, 3), use_bias=True))  # with APC
...
#model.save_weights('v6.4_test_result_IntensiveTests_17.h5')
model.load_weights('v6.4_test_result_IntensiveTests_17.h5')
...
for r in range(10):
    ...
    txtTitle = 'v6.4_intensive_snn_tests17_conv_SNN1_' + str(test_index+1) + '.txt'
    ...
    txtTitle = 'v6.4_intensive_snn_tests17_conv_BNN1_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    hoConvLayer = HOConvolution(3, 3, length, baseMode="Mux", activationFunc="Relu", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    ...
    # Convert Stochastic number to Binary number
    conv_out_test = np.zeros((5, 23, 23))
    for i in range(5):
        for j in range(23):
            for k in range(23):
                conv_out_test[i, j, k] = stochtoint(conv_output[i, j, k])
    ...
    # Write the array to disk
    txtTitle = 'v6.4_intensive_snn_tests17_conv_SNN2_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(conv_out_test.shape))
    ...
    txtTitle = 'v6.4_intensive_snn_tests17_conv_BNN2_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
    ...
    # Convert Stochastic number to Binary number
    max_pool_out_test = np.zeros((5, 11, 11))
    for i in range(5):
        for j in range(11):
            for k in range(11):
    ...
    # Write the array to disk
    txtTitle = 'v6.4_intensive_snn_tests17_maxpool_SNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    txtTitle = 'v6.4_intensive_snn_tests17_maxpool_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))

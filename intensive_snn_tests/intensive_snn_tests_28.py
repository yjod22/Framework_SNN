...
#              Conv(4, 4x4, non-bias), activation function(tanh), maxPooling(2x2), Dense, activation function(softmax)
#              Stochastic Conv(Mux+STanh), Stochastic Dense(Normal mode)
...
txtTitle = 'v8.0_intensive_snn_tests28_conv_BNN_1st_model' + str(0 + 1) + '.txt'
...
ut = HOUtils()
length = 1024
temp1, temp2 = ut.GetConvolutionLayerWeightsSN(model, 1, length, 1)
del(temp1)
del(temp2)
del(ut)
...
#model2.save_weights('v8.0_test_result_IntensiveTests_20.h5')
model2.load_weights('v8.0_test_result_IntensiveTests_20.h5')
...
# Predict the intermediate results from the Binary Neural Network
BNN_prediction = layer2model.predict(np.asarray([x_test[0]]))
txtTitle = 'v8.0_intensive_snn_tests28_conv_BNN_2nd_model' + str(0 + 1) + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
...
weight_SNs, listIndex = ut.GetConvolutionLayerWeightsSN(model2, 1, length, 2)
...
for r in range(10):
    ...
    conv_out_test = np.zeros((4, 25, 25))
    for i in range(4):
        for j in range(25):
            for k in range(25):
    ...
    txtTitle = 'v8.0_intensive_snn_tests28_conv_SNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    txtTitle = 'v8.0_intensive_snn_tests28_conv_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
    ...
    # Convert Stochastic number to Binary number
    max_pool_out_test = np.zeros((4, 12, 12))
    for i in range(4):
        for j in range(12):
            for k in range(12):
    ...
    # Write the array to disk
    txtTitle = 'v8.0_intensive_snn_tests28_maxpool_SNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt

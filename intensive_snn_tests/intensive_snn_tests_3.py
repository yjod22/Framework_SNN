...
# Change:      snLength: 2048
#              Conv(3, 4x4), activation function(tanh(0.7*x), maxPooling(2x2), Dense, activation function(softmax)
#              Stochastic Conv(APC+BTanh), Stochastic Dense(Normal mode)
# Cause:       Need short description for this file
# Initiator:   Junseok Oh
################################################################################
# File:		   intensive_snn_tests.py
# Version:     6.2 (SCR_V6.1-1)
# Author/Date: Junseok Oh / 2019-02-19
# Change:      Change the input of tanh function
#			   Use "BTanh" as the activation function of first convolution layer 
...
#model.save_weights('v6.2_test_result_IntensiveTests_3_tanh0_7.h5')
model.load_weights('v6.2_test_result_IntensiveTests_3_tanh0_7.h5')
...
for r in range(10):
    ...
    conv_out_test = np.zeros((3, 25, 25))
    for i in range(3):
        for j in range(25):
            for k in range(25):
    ...
    txtTitle = 'v6.2_intensive_snn_tests3_conv_SNN_' + str(test_index+1) + '.txt'
    ...
    txtTitle = 'v6.2_intensive_snn_tests3_conv_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    # Convert Stochastic number to Binary number
    max_pool_out_test = np.zeros((3, 12, 12))
    for i in range(3):
        for j in range(12):
            for k in range(12):
    ...
    # Write the array to disk
    txtTitle = 'v6.2_intensive_snn_tests3_maxpool_SNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    txtTitle = 'v6.2_intensive_snn_tests3_maxpool_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
    ...
    hoModel.SetZeroDenseBias(10)
    hoDenseLayer = HOConnected(length, stochToInt="Normal", activationFunc="None", use_bias="False")

...
###############################################################################
#
#  Filename:     WeightScaling_testCase7_MNIST.py
#
###############################################################################
#  Description:
#
...
class WeightScale(Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        if(global_variables.bnModel.GetId() == 1):
            global_variables.bnModel.Save_weights('../results/#Epoch' + str(global_variables.cntEpochs)+'_weights_of_1st_model_testCase7_MNIST.h5')
        elif(global_variables.bnModel.GetId() == 2):
            global_variables.bnModel.Save_weights('../results/#Epoch' + str(global_variables.cntEpochs)+'_weights_of_2nd_model_testCase7_MNIST.h5')
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        weights_dense2 = self.model.get_layer(index=10).get_weights()
        maximum = 1
        ...
        for w in weights_dense2:
            maximumDense2 = max(np.max(np.absolute(w)), maximumDense2)
        ...
        self.model.get_layer(index=10).set_weights([w/maximumDense2 for w in weights_dense2])
...
# Version:     19.2
# Author/Date: Junseok Oh / 2020-01-17
# Change:      (SCR_V19.1-1): Test with MNIST
# Cause:       -
...
import WeightScaling_testCase6_MNIST
import numpy as np
...
batch_size = 128
num_classes = 10
num_1st_dense_output_neuron = 100
epochs = 25
cntEpochs = 0
...
global_variables.bnModel[4] = Activation(second_layer_activation)
global_variables.bnModel[5] = MaxPooling2D(pool_size=(2, 2))
global_variables.bnModel[6] = Flatten()
global_variables.bnModel[7] = Dense(num_1st_dense_output_neuron)
global_variables.bnModel[8] = Activation('relu')
global_variables.bnModel[9] = Dense(num_classes)
global_variables.bnModel[10] = Activation('softmax')
...
#                             epochs=epochs,
#                             verbose=0,
#                             callbacks=[WeightScaling_testCase6_MNIST.WeightScale()],
#                             validation_data=(x_test, y_test))
global_variables.bnModel.Load_weights('../results/#Epoch25_weights_of_1st_model_testCase6_MNIST.h5')
global_variables.bnModel.Evaluate(x_test[:800], y_test[:800], verbose=0, indexModel=1)

# Optimize the neural network
#global_variables.bnModel.OptimizeNetwork('testCase6_MNIST',
#                                         '../results/#Epoch25_weights_of_1st_model_testCase6_MNIST.h5',
#                                         '../results/#Epoch1_weights_of_1st_model_testCase6_MNIST.h5',
#                                         WeightScaling_testCase6_MNIST,
#                                         cntIter=3,
#                                         tupleLayer=(1, ),
#                                         x_train=x_train,
#                                         y_train=y_train,
...
global_variables.bnModel.Load_weights('../results/#Epoch24_weights_of_2nd_model_testCase6_MNIST.h5')
...
layer10model = global_variables.bnModel[9]
layer11model = global_variables.bnModel[10]
...
# extract weights and biases from the trained BNN
...
dense2_biases = ut.GetConnectedLayerBiases(model, 10)
dense2_weights = ut.GetConnectedLayerWeights(model, 10)
...
for r in range(iter_validation):
    ...
    hoConvLayer = HOConvolution(4, 4, kBits=kBits, baseMode="APC", activationFunc="BTanh", use_bias="False")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print('conv layer 1 done')

    if(test_index % 100 == 0):
        ut.SaveInTxtFormat('../results/v19.2_testCase10_MNIST_conv1', test_index,
    ...
    hoConvLayer = HOConvolution(5, 5, kBits=kBits, baseMode="APC", activationFunc="BTanh", use_bias="False", scale=0.4)
    hoModel.Activation(hoConvLayer, stride=1)
    del (hoConvLayer)
    print("conv layer 2 done")

    if (test_index % 100 == 0):
        ut.SaveInTxtFormat('../results/v19.2_testCase10_MNIST_conv2', test_index,
    ...
    hoDenseLayer = HOConnected(kBits=kBits, stochToInt="Normal", activationFunc="Relu", use_bias="True")
    hoModel.Activation(hoDenseLayer, num_classes=num_1st_dense_output_neuron)
    del(hoDenseLayer)
    ...
    # Second dense layer
    dense_output = hoModel.GetOutputMatrix()
    dense_output = ut.BinaryConnectedLAyer(num_1st_dense_output_neuron, num_classes,
                                           dense_output, dense2_weights, dense2_biases)

    ################### For debugging purpose, save the intermidiate results in the local variable ###################
    print("dense 2 output from Binary NN without softmax")
    BNN_prediction = layer10model.predict(np.asarray([x_test[test_index]]))
    print(BNN_prediction)
    del (BNN_prediction)
    ...
    out_error = 0
    out = layer10model.predict(np.asarray([x_test[test_index]]))
    for i in range(10):

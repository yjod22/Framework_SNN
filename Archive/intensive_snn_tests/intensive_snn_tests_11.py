...
#              Conv(1, 1x1), activation function(tanh(x)), Conv(10, 1x1), activation function(tanh(x)),
#              maxPooling(2x2), Dense(100), activation function(relu), Dense(10), activation function(softmax)
#              Stochastic Conv(Mux+STanh, Non-adaptive), Stochastic Conv(Mux+STanh, Non-adaptive), 
#			   Stochastic Dense(APC mode+Relu), BinaryConnectedLAyer
...
# Change:      snLength: 4096
#              Conv(1, 1x1), activation function(tanh(x/2)), Conv(10, 1x1), activation function(tanh(x/2)),
#              maxPooling(2x2), Dense(100), activation function(relu), Dense(10), activation function(softmax)
#              Stochastic Conv(Mux+STanh), Stochastic Conv(Mux+STanh), Stochastic Dense(APC mode+Relu), BinaryConnectedLAyer
...
model.add(Activation('second_layer_activation'))
...
#model.save_weights('v8.0_test_result_IntensiveTests_11.h5')
model.load_weights('v8.0_test_result_IntensiveTests_11.h5')
...
#weight_1_SNs, listIndex1 = ut.GetConvolutionLayerWeightsSNNonAdaptive(model, 1, length)
...
#weight_3_SNs, listIndex3 = ut.GetConvolutionLayerWeightsSNNonAdaptive(model, 3, length)
...
for r in range(10):
    ...
    hoConvLayer = HOConvolution(1, 1, length, baseMode="Mux", activationFunc="STanh", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print("conv 1 layer done")

    ut.SaveInTxtFormat('v8.0_intensive_snn_tests11_conv1', test_index,
                       hoModel.GetOutputMatrix(), 1, 28, 28,
                       layer2model, x_test)
    ...
    ut.SaveInTxtFormat('v8.0_intensive_snn_tests11_conv2', test_index,
                       hoModel.GetOutputMatrix(), 10, 28, 28,
                       layer4model, x_test)
    ...
    ut.SaveInTxtFormat('v8.0_intensive_snn_tests11_maxpool', test_index,
                       hoModel.GetOutputMatrix(), 10, 14, 14,
                       layer5model, x_test)

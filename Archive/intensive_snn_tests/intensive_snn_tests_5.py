...
#              Stochastic Conv(Mux+Relu), Stochastic Dense(APC mode)
...
#model.save_weights('v6.1_test_result_IntensiveTests_5.h5')
model.load_weights('v6.1_test_result_IntensiveTests_5.h5')
...
for r in range(10):
    ...
    txtTitle = 'v6.1_intensive_snn_tests5_conv_SNN_' + str(test_index+1) + '.txt'
    ...
    txtTitle = 'v6.1_intensive_snn_tests5_conv_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    # Write the array to disk
    txtTitle = 'v6.1_intensive_snn_tests5_maxpool_SNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
    ...
    txtTitle = 'v6.1_intensive_snn_tests5_maxpool_BNN_' + str(test_index+1) + '.txt'
    with open(txtTitle, 'w') as outfile:
        # I'm writing a header here just for the sake of readability
        # Any line starting with "#" will be ignored by numpy.loadtxt
        outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
    ...
    hoDenseLayer = HOConnected(length, stochToInt="APC", activationFunc="None")

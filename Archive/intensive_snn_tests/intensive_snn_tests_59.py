...
#              Conv(2, 5x5), activation function(Clipped-ReLU), maxPooling(2x2), Dense(10), activation function(softmax)
#              Stochastic Conv(APC+SC-based ReLU), Stochastic Dense(Normal mode+None)
#              (SCR_V9.0-4): Verify new APCs, SC-based ReLU
# Cause:       -
...
import WeightScaling_intensiveTests_59
...
global_variables.bnModel[0] = Conv2D(2, kernel_size=(5, 5),
...
global_variables.bnModel.Fit(x_train, y_train,
                             ...
                             epochs=epochs,
                             verbose=0,
                             callbacks=[WeightScaling_intensiveTests_59.WeightScale()],
                             validation_data=(x_test, y_test))
global_variables.bnModel.Load_weights('results/#Epoch5 weights of 1st model_tests59.h5')
...
for r in range(30):
    ...
    hoConvLayer = HOConvolution(5, 5, length, baseMode="APC", activationFunc="SCRelu", use_bias="False")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print('conv layer 1 done')

    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v10.0_intensive_snn_tests59_conv1', test_index,
                           hoModel.GetOutputMatrix(), 2, 24, 24,
                           layer2model, x_test)
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v10.0_intensive_snn_tests59_maxpool', test_index,
    ...
    hoDenseLayer = HOConnected(length, stochToInt="Normal", activationFunc="None", use_bias="True")

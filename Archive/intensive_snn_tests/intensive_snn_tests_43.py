...
txtTitle = 'v8.0_intensive_snn_tests43_conv_BNN_1st_model' + str(0 + 1) + '.txt'
...
txtTitle = 'v8.0_intensive_snn_tests43_weights1' + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
# Predict the intermediate results from the Binary Neural Network
BNN_prediction = layer2model.predict(np.asarray([x_test[0]]))
txtTitle = 'v8.0_intensive_snn_tests43_conv_BNN_2nd_model' + str(0 + 1) + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('# Array shape: {0}\n'.format(BNN_prediction[0].shape))
...
txtTitle = 'v8.0_intensive_snn_tests43_weights2' + '.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('# Array shape: {0}\n'.format(weights.shape))
...
for r in range(100):
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests43_conv', test_index,
    ...
    sentences += '\n' + 'maxpool layer done'
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests43_maxpool', test_index,
...
################### For debugging purpose, save the weigts in txt format #######################################
txtTitle = 'v8.0_Intensive_snn_tests43.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    #outfile.write('# Array shape: {0}\n'.format(weights.shape))

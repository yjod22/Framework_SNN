...
#              epoch: 2
#              # of test cases: 100
#              Conv(4, 4x4, non-bias), activation function(tanh), maxPooling(2x2),
#              Dense, activation function(softmax)
#              Stochastic Conv(Mux+STanh(Non-Adaptive)), Stochastic Dense(Normal mode)
...
# Generate the weights in txt format
weights = model.get_layer(index=1).get_weights()[0]
txtTitle = 'v8.0_intensive_snn_tests34_weights1' + '.txt'
...
weight_SNs, listIndex = ut.GetConvolutionLayerWeightsSNNonAdaptive(model, 1, length)
...
for r in range(100):
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests34_conv', test_index,
    ...
    sentences += '\n' + 'maxpool layer done'
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('v8.0_intensive_snn_tests34_maxpool', test_index,
...
txtTitle = 'v8.0_Intensive_snn_tests34.txt'
with open(txtTitle, 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    #outfile.write('# Array shape: {0}\n'.format(weights.shape))

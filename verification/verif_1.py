...
from verification import WeightScaling_verif_1
import numpy as np
...
global_variables.bnModel.Fit(x_train, y_train,
                             ...
                             epochs=epochs,
                             verbose=0,
                             callbacks=[WeightScaling_verif_1.WeightScale()],
                             validation_data=(x_test, y_test))
global_variables.bnModel.Load_weights('../results/#Epoch10_weights_of_1st_model_verif_1.h5')
...
for r in range(10):
    ...
    hoConvLayer = HOConvolution(4, 4, kBits=kBits, baseMode="APC", activationFunc="SCRelu", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print('conv layer 1 done')

    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_1_conv1', test_index,
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_1_maxpool', test_index,

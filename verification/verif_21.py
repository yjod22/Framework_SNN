...
from verification import WeightScaling_verif_21
import numpy as np
...
global_variables.bnModel[0] = Conv2D(4, kernel_size=(3, 3),
...
global_variables.bnModel.Fit(x_train, y_train,
                             ...
                             epochs=epochs,
                             verbose=0,
                             callbacks=[WeightScaling_verif_21.WeightScale()],
                             validation_data=(x_test, y_test))
global_variables.bnModel.Load_weights('../results/#Epoch10_weights_of_1st_model_verif_21.h5')
...
for r in range(iter_validation):
    ...
    hoConvLayer = HOConvolution(3, 3, kBits=kBits, baseMode="Mux", activationFunc="Relu", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print('conv layer 1 done')

    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_21_conv1', test_index,
                           hoModel.GetOutputMatrix(), 4, 26, 26,
                           layer2model, x_test)
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_21_maxpool', test_index,
                           hoModel.GetOutputMatrix(), 4, 13, 13,
                           layer3model, x_test)

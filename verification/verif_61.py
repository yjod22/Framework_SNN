...
from verification import WeightScaling_verif_61
import numpy as np
...
global_variables.bnModel[2] = Conv2D(2, kernel_size=(3, 3),
                                     input_shape=input_shape,
                                     use_bias=True,
                                     kernel_regularizer=regularizers.l1(0.0007))
...
global_variables.bnModel.Fit(x_train, y_train,
                             ...
                             epochs=epochs,
                             verbose=0,
                             callbacks=[WeightScaling_verif_61.WeightScale()],
                             validation_data=(x_test, y_test))
global_variables.bnModel.Load_weights('../results/#Epoch20_weights_of_1st_model_verif_61.h5')
...
global_variables.bnModel.OptimizeNetwork('verif_61',
                                         '../results/#Epoch20_weights_of_1st_model_verif_61.h5',
                                         '../results/#Epoch1_weights_of_1st_model_verif_61.h5',
                                         WeightScaling_verif_61,
                                         cntIter=1,
                                         tupleLayer=(1, 3),
                                         x_train=x_train,
                                         y_train=y_train,
...
global_variables.bnModel.Evaluate(x_test[:500], y_test[:500], verbose=0, indexModel=2)
global_variables.bnModel.Evaluate(x_test[:1000], y_test[:1000], verbose=0, indexModel=2)
...
weight_2_SNs, bias_2_SNs, listIndex2 = ut.GetConvolutionLayerWeightsBiasesSN(model, 3, Adaptive="True")

dense_biases = ut.GetConnectedLayerBiases(model, 7)
dense_weight_SNs = ut.GetConnectedLayerWeightsSN(model, 7)
...
for r in range(10):
    ...
    hoConvLayer = HOConvolution(4, 4, kBits=kBits, baseMode="Mux", activationFunc="STanh", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del(hoConvLayer)
    print('conv layer 1 done')

    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_61_conv1', test_index,
    ...
    hoConvLayer = HOConvolution(3, 3, kBits=kBits, baseMode="Mux", activationFunc="STanh", use_bias="True")
    hoModel.Activation(hoConvLayer, stride=1)
    del (hoConvLayer)
    print("conv layer 2 done")

    if (test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_61_conv2', test_index,
                           hoModel.GetOutputMatrix(), 2, 23, 23,
                           layer4model, x_test)
    ...
    if(test_index % 10 == 0):
        ut.SaveInTxtFormat('../results/v15.0_verif_61_maxpool', test_index,
                           hoModel.GetOutputMatrix(), 2, 12, 12,
                           layer3model, x_test)
